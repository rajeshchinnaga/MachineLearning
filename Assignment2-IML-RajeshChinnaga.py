...
if __name__ == '__main__':
    ...
    for i in range(0,1000,1):
        axes[0,0].plot(xV[0][i], yV[i], color='red', marker='.', alpha=0.4)
        axes[0,1].plot(xV[1][i], yV[i], color='green', marker='.', alpha=0.4)
        axes[1,0].plot(xV[0][i], yV[i], color='blue', marker = '.', alpha=0.4)
    ...
    print(MSE)
    gamma_values = np.geomspace(1e-4, 1e4, 9) 
    print(gamma_values)
    
    fig, ax = plt.subplots()
    plt.plot(gamma_values, MSE)
    ax.set_xscale('log')
    plt.xlabel('Gamma values')
    plt.ylabel('MSE')
    plt.title('MSE for Varied Gamma')
    plt.grid(True)
    plt.tight_layout()
    plt.show
    
    #MAP AND MLE and corresponding data plots.
